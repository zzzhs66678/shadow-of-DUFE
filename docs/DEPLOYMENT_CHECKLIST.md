...
- [ ] GitHub `postgres-17-integration` 作业从空库执行 `run-migrations.sh` 两遍，通过真实 owner/runtime/importer 连接测试，并用两个普通账号和一个管理员走通注册/会话、个人同步幂等/冲突/账号隔离、教师评价跨账号隔离/乐观并发、普通用户后台拒绝、主题/回复/通知、bootstrap 撤销旧会话、TOTP 提升、举报证据/入案/隐藏与审计；本地 SKIP 不得作为替代证据。
