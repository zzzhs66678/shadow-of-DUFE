...
- [ ] 关键路径浏览器 E2E 通过：注册→资料→跨设备→教师→评价→回复→通知→举报→管理员处理。
  - 完整 PG17 浏览器路径已经编码并可被 Playwright 收集，但尚未在真实数据库运行；首次 CI 全绿前不得勾选。
...
- 本地 `npm run typecheck`、主仓 Node 128 项中 125 通过且 3 项 PostgreSQL 明确 SKIP、个人存储/同步 9/9、auth-api 75/75、构建后渲染 4/4、CSS 审计和全仓 ESLint 已通过；渲染 4 项已包含在主仓 128 项中，不重复计数。quality workflow 已加入 TypeScript 步骤；仍须由 Pull Request CI 在 Linux 上复核后才能勾选生产前质量门禁。
