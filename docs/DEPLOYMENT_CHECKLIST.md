...
- [ ] 教师私有包必须显式记录具体学年；导入后随机抽查教师详情中的教学班与教材均显示 `YYYY-YYYY-fall/spring` 对应的完整学年学期，且任何已解析教师教材都能形成同一教师的教学班事实，不出现“有教材、教学班为 0”。
...
- [x] Linux 隔离 staging 容器逐项请求 HTML 引用的 JS/CSS 哈希资源并确认 200、正确 MIME 与长期缓存；PR #10 run `31687272428` 已通过。外部 staging 仍须用正式域名和镜像摘要复跑并记录，归入第 6 节整体环境验收。
...
- [x] 使用独立 staging 数据完成迁移，不复制生产敏感数据；当前没有获批的私有种子包，因此未执行私有教师/教材种子导入。
- [ ] HTTPS、域名和核心健康检查已真实工作；邮件、外部回调、上传、私有资料预览和真实通知仍待对应凭据/测试数据。
- [ ] 至少两个普通账号和一个管理员完成端到端验收。
- [ ] 负载下会话、搜索、社区和同步无明显错误；数据库连接、CPU、内存、磁盘可观测。
- [ ] 告警能覆盖 5xx、登录异常、任务积压、磁盘、备份失败和数据库不可用。
- [x] staging 部署地址、提交 SHA、镜像摘要与测试时间写入 `PROGRESS.md`。
...
- 当前开发机未安装 `sh`、Docker/PostgreSQL 或 WSL；PR #10 run `31687272428` 已完成空库 20 个迁移双执行、角色 ACL、课程教师覆盖、评价回复治理、完整真实账号浏览器路径、现场备份恢复、Linux musl 原生模块，以及隔离 Compose 实际启动与 HTTPS 资源冒烟。既有数据库升级副本、真实私有导入包、生产备份和外部服务仍须 staging 验收。
- 主站已用仓库内失败关闭包隔离 vinext 的 `image-size@2.0.2`；Linux PR CI 已复核实际去重、构建、生产审计与网关冒烟。正式镜像摘要和上一版本回滚仍须 staging 记录。
- 本地定向门禁与 PR #10 quality run `31687272428` 均通过；后续代码或发布文档变化必须重新保持 PR CI 全绿。该证据不替代真实 Web Vitals、公开代理链、监控、外部凭据和生产审批。
