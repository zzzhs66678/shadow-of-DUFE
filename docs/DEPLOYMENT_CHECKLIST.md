...
- 当前开发机未安装 `sh`、Docker/PostgreSQL 或 WSL；PR #10 已在 GitHub runner 完成空库 17 个迁移双执行、角色 ACL/共享限流、完整真实账号浏览器路径、现场备份恢复和 Linux musl 原生模块验证。尚未完成的是既有数据库升级副本、真实私有导入包、生产备份和外部服务的 staging 验收。
