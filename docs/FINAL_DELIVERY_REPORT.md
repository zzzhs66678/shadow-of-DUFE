...
- `已实现待 staging`：隔离 Compose 已在 Linux CI 真正启动并完成 HTTPS/API/JS/CSS 冒烟；既有数据库升级副本、正式镜像摘要与上一版本回滚、真实 Web Vitals/公开代理/监控、生产备份和外部服务仍需独立环境验证。
...
- `已实现待 staging`：GitHub Linux 镜像、PG17 双迁移/ACL/并发、完整浏览器链、隔离 Compose 实际启动、HTTPS/API/前端资源冒烟、CodeQL、全历史 Gitleaks 和现场备份恢复已在 run `31687272428` 通过；尚需既有库升级、正式镜像摘要和上一版本不可变回滚演练。
