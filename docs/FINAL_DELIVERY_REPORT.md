...
- `已完成并本地验证`：生产 SMTP 适配器强制 TLS 1.2+ 和证书验证，禁用文件/URL/附件读取，生产不回传令牌；密码重置不等待 SMTP 网络，投递失败保持统一 202 防枚举。
...
- 最新本地总回归：主仓 Node 128 项中 125 通过、3 项 PostgreSQL 明确 SKIP；个人存储/同步 9/9；auth-api 75/75；小影 35/35。
