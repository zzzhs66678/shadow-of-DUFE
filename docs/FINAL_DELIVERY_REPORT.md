...
状态口径：`已完成并本地验证`、`已实现待环境验证`、`外部阻塞`、`尚未实现`。本报告不把静态合同、明确 SKIP 或旧正式站状态冒充新版本生产证据。
...
- `已完成并本地验证`：保留“今日学习台”为首要任务面，采用纸、墨、朱砂与校园时间轨原创系统；校园照片集中在“我的”，社区和教师作为独立工作区。
- `已完成并本地验证`：凭据账号、个人云同步、资料 API、教师 UUID、用户评价、社区/通知/治理、管理员 MFA、共享限流、安全网关和小影隔离均有真实服务端实现，不以按钮隐藏或内存假数据充数。
- `已实现待环境验证`：PostgreSQL 迁移、角色 ACL、完整账号浏览器链、Linux 镜像、备份恢复和不可变发布均已编码进 CI/运维流程，但本开发机没有 PostgreSQL、Docker、sh 或 WSL。
- `外部阻塞`：真实微信、邮件投递、独立 staging 和异地备份仍需外部账号或凭据。
...
- `已完成并本地验证`：用户名或邮箱登录、Argon2id、统一错误、失败锁定、单次重置/验证令牌、Secure/HttpOnly Cookie、头像规范化和跨账号本地隔离。
- `已完成并本地验证`：云端 revision、稳定客户端 ID、幂等 mutation、三方合并、冲突保留、设备撤销和账号注销均有 API/存储测试。
- `已完成并本地验证`：后台所有 API 先做服务端角色检查，高风险操作要求绑定基础会话的短期 TOTP/恢复码提升，并写不可变审计。
- `已实现待环境验证`：完整浏览器用例覆盖三个注册账号、第二设备资料/头像、普通用户拒绝、管理员 bootstrap、实时 TOTP 和举报处置；尚未连接真实 PG17 执行。
...
- `已完成并本地验证`：统一纸白、炭墨、朱砂，松针绿只表达状态；不再使用蓝色主视觉、随机圆角或通用 SaaS 卡片语言。
...
- 最新本地总回归：主仓 Node 127 项中 124 通过、3 项 PostgreSQL 明确 SKIP；个人存储/同步 9/9；auth-api 72/72；小影 35/35。
...
- `已实现待环境验证`：SEC-002/003/006/009/010/011/013/015/016/017/018 仍含真实 PG、Linux、runner、服务器权限或 staging 证据，不能宣布全部 P0/P1 已关闭。
...
- `已实现待环境验证`：LCP≤2.5s、INP≤200ms、CLS≤0.1 只作为 staging 预算，本地文件体积与 Playwright 不能冒充真实 Web Vitals。
...
- `尚未实现`：生产邮件投递适配器；密码登录可用，但生产重置与邮箱验证邮件在取得服务凭据前保持 disabled。
- `外部阻塞`：独立 staging 域名/数据库/服务器凭据和异地 OSS 备份凭据尚未提供。
- `已实现待环境验证`：GitHub Linux 镜像、PG17 双迁移/ACL/并发、完整浏览器链、CodeQL、全历史 Gitleaks、现场备份恢复和不可变镜像回滚尚未运行。
- `已实现待环境验证`：真实教师/教材私有导入包仍须在 staging 走 dry-run、apply、审核和回滚；未审核历史评价不得公开。
- `已实现待环境验证`：真实 Web Vitals、代理链、域名跳转、CSP 控制台、系统监控与管理员一次性初始化需 staging/生产主机验证。
- `尚未完成`：分支推送、Pull Request 审查与合并；在获得明确发布授权前不执行。
